...
import { $vxBackdrop } from '../index';

baseComponent({
    useSafeArea: true,
    externalClasses: ['vx-content-class', 'vx-header-class', 'vx-body-class', 'vx-footer-class', 'vx-close-class'],
    properties: {
        prefixCls: {
            type: String,
            value: 'vx-popup'
    ...
    computed: {
        ss: [
            'prefixCls, position, isIPhoneX',
            function (prefixCls, position, isIPhoneX) {
                const wrap = classNames(`${prefixCls}-position`, {
                    [`${prefixCls}-position--${position}`]: position,
                    [`${prefixCls}-position--is-iphonex`]: isIPhoneX
                });
    ...
    methods: {
        ...
        setBackdropVisible(visible) {
            if (this.data.mask && this.$vxBackdrop) {
                this.$vxBackdrop[visible ? 'retain' : 'release']();
            }
        }
    },
    created() {
        if (this.data.mask) {
            this.$vxBackdrop = $vxBackdrop('#vx-backdrop', this);
        }
    },
    attached() {
